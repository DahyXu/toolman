...
<p>That capital <code>X</code> is worth remembering. <code>chmod -R 755</code> on a source tree marks every file executable, which is wrong for almost all of them; <code>chmod -R u+rwX</code> does the sensible thing instead.</p>`,
  faq: [
    ...
    { q: 'What does the x bit do on a directory?', a: 'The execute bit on a directory grants the right to traverse it and reach the entries inside it by name. Without it, the directory cannot be entered even if you can list its contents, which is why directories need execute and ordinary files do not.' },
