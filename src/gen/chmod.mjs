...
export default async function () {
  ...
  for (const v of VALUES) {
    ...
    const FAQ = faq([
      { q: `What does chmod ${v} mean?`, a: `${d === '000' ? 'It grants <strong>no access to anyone</strong> — not even the owner, who can still change the permissions back because that right comes from ownership rather than from these bits' : `It sets ${who.map((w, i) => `<strong>${w}</strong> to ${words(+d[i])}`).join(', ')}`}${special ? `, with the ${[(special & 4) && 'setuid', (special & 2) && 'setgid', (special & 1) && 'sticky'].filter(Boolean).join(' and ')} bit set` : ''}. In symbolic notation that is <code>${sym}</code>.` },
