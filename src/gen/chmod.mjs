...
export default async function () {
  ...
  for (const v of VALUES) {
    ...
    const FAQ = faq([
      { q: `What does chmod ${v} mean?`, a: `${d === '000' ? `chmod ${v} grants <strong>no access to anyone</strong> — not even the owner, who can still change the permissions back because that right comes from ownership rather than from these bits` : `chmod ${v} sets ${who.map((w, i) => `<strong>${w}</strong> to ${words(+d[i])}`).join(', ')}`}${special ? `, with the ${[(special & 4) && 'setuid', (special & 2) && 'setgid', (special & 1) && 'sticky'].filter(Boolean).join(' and ')} bit set` : ''}. In symbolic notation that is <code>${sym}</code>.` },
      ...
      { q: `Is ${v} safe?`, a: worldWritable ? `chmod ${v} is world-writable, so any account on the system can modify the contents. That is only appropriate for a shared scratch area${v === '1777' ? ', which is why /tmp uses it together with the sticky bit' : ', and even then the sticky bit should normally be set as well'}. For anything else, grant access through a group instead.` : special & 4 ? `Setuid means the program runs with its owner’s privileges regardless of who starts it. That is a deliberate privilege escalation and should exist only on binaries that were written to be safe under it.` : `chmod ${v} grants no write access outside ${(+d[1] & 2) ? 'the owner and the group' : 'the owner'}, so it is a reasonable setting as long as the owner is the account you intend.` },
